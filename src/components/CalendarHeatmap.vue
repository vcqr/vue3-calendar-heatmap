				...
				<g class="vch__month__wrapper" v-for="(week, weekIndex) in heatmap.calendar" :key="weekIndex"
					...
					<template v-for="(day, dayIndex) in week" :key="dayIndex">
						<rect class="vch__day__square" v-if="day.date < now" :rx="round" :ry="round"
							...
							:data-tippy-content="tooltipOptions(day)" @click="$emit('dayClick', day)" />
					</template>
				</g>
			</g>
		</svg>
...
export default /*#__PURE__*/defineComponent({
	...
	props: {
		...
		noDataText: {
			type: Function as PropType<TooltipFormatter> | StringConstructor | BooleanConstructor
		},
		round: {
			type: Number,
			default: 0
		...
		function tooltipOptions(day: CalendarItem) {
			if (props.tooltip) {
				if (day.count !== undefined) {
					...
					if (typeof props.noDataText == 'function') {
						return props.noDataText(day, props.tooltipUnit);
					}

					if (typeof props.noDataText == 'string') {
